...
import { Settings, CheckCircle2, Factory, Gauge, Thermometer, Clock } from 'lucide-react';
...
export default function PremiumProcessPage() {
  ...
  const processSteps = [
    {
      ...
      [cite_start]desc: "The coal fly ash will be sent to the slurry preparing pond by wheel loader to be prepared into the required concentration coal fly ash slurry by adding rated water, which will be pumped into a slurry tank to be stored and ready to be used [cite: 115-116].",
    },
    {
      id: 2,
      phase: 1,
      title: "2. Lime",
      [cite_start]desc: "The powder will be sent into the lime powder silo for storage and ready to be used by bucket elevator, after coming into the factory [cite: 117-118].",
    },
    {
      id: 3,
      phase: 1,
      title: "3. Gypsum",
      [cite_start]desc: "The gypsum in the storage shed will be added into the slurry pond according to the required proportion, while preparing slurry [cite: 119-120].",
    },
    {
      id: 4,
      phase: 1,
      title: "4. Cement",
      [cite_start]desc: "The cement will be transported into the factory by tank truck and directly is pumped into the cement silo to be used [cite: 121-122].",
    },
    {
      id: 5,
      phase: 1,
    ...
    },
    {
      ...
      [cite_start]desc: "After unloading, the side plate on the trolley will be lifted to the returning rail by crane, returned to the side of the cutting machine, combined with the mould, and sent to the mould returning line for cleaning and oiling to be reused [cite: 147-148].",
    }
  ];
  ...
  const getVideoForPhase = (phase: number) => {
    switch(phase) {
      case 1: return "/video-materials.mp4"; // 🎬 Prompt: Top-down macro of raw materials being sorted
      case 2: return "/video-mixing.mp4";    // 🎬 Prompt: High-speed industrial mixing of slurry
      case 3: return "/video-cutting.mp4";   // 🎬 Prompt: Precision wire cutting the green block
      case 4: return "/video-autoclave.mp4"; // 🎬 Prompt: Steam venting from massive industrial autoclave
      default: return "/video-materials.mp4";
    }
  };
    ...
    <div className="font-sans text-slate-900 bg-white min-h-screen pt-24 pb-20">
      ...
      <section className="py-20 px-6 md:px-12 max-w-7xl mx-auto text-center">
        ...
        >
          ...
          <p className="text-xl md:text-2xl text-slate-500 max-w-3xl mx-auto font-light leading-relaxed">
            A state-of-the-art, fully automated process engineered for absolute uniformity, zero wastage, and maximum structural integrity.
          </p>
        </motion.div>
      </section>
          ...
          <div className="lg:w-1/2 space-y-8 pb-32">
            ...
            <div className="border-l-2 border-slate-100 pl-8 mt-12 space-y-16">
              {processSteps.map((step) => (
                ...
                >
                  {/* Timeline Node */}
                  <div className={`absolute -left-[41px] top-0 w-4 h-4 rounded-full border-4 border-white shadow-sm transition-colors duration-500 ${activePhase === step.phase ? 'bg-blue-600' : 'bg-slate-300'}`}></div>
                    ...
                    <div className="mt-8 bg-white p-6 rounded-2xl border border-slate-200 shadow-lg group cursor-crosshair hover:border-blue-500 transition-colors">
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div className="flex flex-col items-center text-center">
                          <Clock className="text-blue-600 mb-2 group-hover:animate-pulse" size={24} />
                          <span className="text-xl font-black text-slate-900">12h</span>
                          <span className="text-xs text-slate-500 uppercase">Curing</span>
                        </div>
                        <div className="flex flex-col items-center text-center border-l border-r border-slate-100 px-2">
                          <Gauge className="text-blue-600 mb-2 group-hover:rotate-45 transition-transform duration-700" size={24} />
                          <span className="text-xl font-black text-slate-900">1.2<span className="text-sm">Mpa</span></span>
                          <span className="text-xs text-slate-500 uppercase">Pressure</span>
                        </div>
                        <div className="flex flex-col items-center text-center">
                          <Thermometer className="text-blue-600 mb-2 group-hover:scale-110 transition-transform duration-500" size={24} />
                          <span className="text-xl font-black text-slate-900">185°C</span>
               ...
               <AnimatePresence mode="wait">
                 ...
                 >
                   <video autoPlay loop muted playsInline className="w-full h-full object-cover opacity-90">
                     <source src={getVideoForPhase(activePhase)} type="video/mp4" />
                   </video>
                 </motion.div>
               </AnimatePresence>
      ...
      <section className="py-24 bg-slate-50 text-center px-6 md:px-12 border-t border-slate-100">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-8">Ready to Procure Enterprise-Grade Blocks?</h2>
          <p className="text-xl text-slate-500 mb-10 font-light">
            Our 10-step precision manufacturing guarantees absolute consistency across bulk orders.
          </p>
          <a 
